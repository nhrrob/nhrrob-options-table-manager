...
- Automated in `.github/workflows/security.yml` (every PR to main): Semgrep `p/php`, PHPStan (`.github/security/phpstan.neon`, level 0 + WordPress stubs), and `.github/security/probe.py` + `endpoint-probe.php`: discovers every plugin-owned AJAX action/REST route, forces nonces valid, calls each as subscriber and anonymous, and fails on any success response or attempted DB write (writes are blocked). See the release skill's Step 0.5 for running it locally.
- Build `ORDER BY` from literals/whitelist comparisons, never from the request value (even validated) — Semgrep's taint analysis flags `$columns[ intval( $_GET[...] ) ]`. Prepare each query exactly once; never embed an already-prepared fragment in another `prepare()`.
