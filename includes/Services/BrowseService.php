<?php
/**
 * Read/delete service backing the Browse REST controller.
 *
...
class BrowseService {
	...
	/**
	 * Activity feed logger used to record saves and deletes.
	 *
	 * @var ActivityService
	 */
	private $activity;
	 ...
	 * @param string $orderby  Sort key (name|size|autoload|id), resolved via resolve_order().
	...
	public function query( $type, $page = 1, $per_page = 20, $search = '', $orderby = 'size', $order = 'desc', $status = '', $owner = '', $user_id = 0, $post_id = 0 ) {
		...
		if ( 'usermeta' === $type ) {
			...
			$result = $this->query_options( $search, $offset, $per_page, $order_sql, $orderby, $order, $owner );
		}
	...
	private function resolve_order( $type, $orderby, $order ) {
		...
		$columns = [
			'options'     => [
				'name'     => 'option_name',
				'size'     => 'LENGTH(option_value)',
				'autoload' => 'autoload',
				'id'       => 'option_id',
			],
			'usermeta'    => [
				'name' => 'meta_key',
				'size' => 'LENGTH(meta_value)',
				'id'   => 'umeta_id',
			],
			'postmeta'    => [
				'name' => 'meta_key',
	...
	/**
	 * Query the wp_options table, excluding transients.
	 *
	 * @param string $search    Search term.
	 * @param int    $offset    Offset.
	 * @param int    $limit     Limit.
	 * @param string $order_sql ORDER BY clause built by resolve_order().
	 * @param string $orderby   Raw sort key — checked here for 'owner', which isn't a real
	 *                          column and can't go through resolve_order()'s ORDER BY map.
	 * @param string $order     Sort direction (asc|desc), used only for the 'owner' in-PHP sort.
	 * @param string $owner     '' or a guessed owner label to filter to (see ScannerManager::guess_owner()).
	 * @return array
	 */
	private function query_options( $search, $offset, $limit, $order_sql = 'ORDER BY LENGTH(option_value) DESC', $orderby = '', $order = 'desc', $owner = '' ) {
		global $wpdb;
		...
		$map_row = function ( $row ) use ( $protected, $scanner ) {
			return [
		...
		// option_name column is read (no value/autoload), and
		// group_owner_names() dedupes to one representative name per prefix
		// before calling guess_owner() — unlike transients (bounded by their
		// own name pattern), wp_options can hold thousands of rows, so a
		// straight per-row guess_owner() port here would turn every Browse
		// visit into a full-table scan.
		$name_sql  = "SELECT option_name FROM {$wpdb->options} $where";
		$all_names = $params ? $wpdb->get_col( $wpdb->prepare( $name_sql, $params ) ) : $wpdb->get_col( $name_sql );
		$owners    = $this->group_owner_names( $all_names, $scanner );

		if ( 'owner' === $orderby || '' !== $owner ) {
			// Owner is guessed per row at read time, not a real DB column, so
			// it can't be filtered/sorted via SQL — fetch every matching row
			// (still via $order_sql, which already sorts correctly unless the
			// requested sort key is itself 'owner'), guess + filter/sort in
			// PHP, then slice out the requested page.
			$sql   = "SELECT option_id, option_name, option_value, autoload FROM {$wpdb->options} $where $order_sql";
			$rows  = $params ? $wpdb->get_results( $wpdb->prepare( $sql, $params ), ARRAY_A ) : $wpdb->get_results( $sql, ARRAY_A );
			$items = array_map( $map_row, $rows ? $rows : [] );
	...
	/**
	 * Distinct owner labels for a list of option names, without calling
	 * guess_owner() once per name — dedupes to one representative name per
	 * leading underscore-delimited segment first (same grouping cost profile
	 * ScannerManager::scan_orphans() already accepts on every Optimize page
	 * load), then guesses only on those representatives.
	 *
	 * @param string[]       $names   Option names.
	 * @param ScannerManager $scanner Reused scanner instance.
	 * @return string[] Sorted, deduplicated owner labels.
	 */
	private function group_owner_names( $names, ScannerManager $scanner ) {
		$representatives = [];
		foreach ( $names as $name ) {
			...
			$key = isset( $parts[0] ) ? $parts[0] : $clean;
		...
		$owners = [];
		foreach ( $representatives as $name ) {
			$owners[ $scanner->guess_owner( $name ) ] = true;
		}
	...
	/**
	 * Query wp_usermeta, joined to wp_users for the owner label.
	 *
	 * @param string $search  Search term.
	 * @param int    $offset  Offset.
	 * @param int    $limit   Limit.
	...
	/**
	 * Query wp_postmeta, joined to wp_posts for the owner label.
	 *
	 * @param string $search  Search term.
	 * @param int    $offset  Offset.
	 * @param int    $limit   Limit.
	...
	/**
	 * Query wp_commentmeta, joined to wp_comments for the owner label.
	 *
	 * @param string $search    Search term.
	 * @param int    $offset    Offset.
	 * @param int    $limit     Limit.
	...
	/**
	 * Query wp_termmeta, joined to wp_terms for the owner label.
	 *
	 * @param string $search    Search term.
	 * @param int    $offset    Offset.
	 * @param int    $limit     Limit.
	...
	/**
	 * Query transients (both `_transient_` and `_site_transient_` scopes),
	 * optionally filtered by status or guessed owner.
	 *
	 * @param string $search    Search term.
	 * @param int    $offset    Offset.
	 * @param int    $limit     Limit.
	 * @param string $order_sql ORDER BY clause built by resolve_order().
	 * @param string $status    '' | active | expired | persistent.
	 * @param string $owner     '' or a guessed owner label (see ScannerManager::guess_owner()).
	 * @return array
	 */
	private function query_transients( $search, $offset, $limit, $order_sql = 'ORDER BY option_id DESC', $status = '', $owner = '' ) {
		...
		$now   = time();
		$items = array_map(
			function ( $row ) use ( $wpdb, $now, $scanner ) {
	...
	private function analyze_value( $raw ) {
		...
		if ( is_serialized( $raw ) ) {
			$data = @unserialize( $raw, [ 'allowed_classes' => false ] ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize,WordPress.PHP.NoSilencedErrors.Discouraged -- allowed_classes:false blocks object injection; @ guards against malformed data slipping past is_serialized()
			if ( is_array( $data ) ) {
	...
	private function encode_for_storage( $value, $format ) {
		if ( 'serialized' === $format ) {
			...
			return serialize( $data ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_serialize -- must match the original option's serialization format
		}
	 ...
	 *
	 * @param array $args Record fields: type, id?, name, value, autoload?, user_id?, expiration?.
