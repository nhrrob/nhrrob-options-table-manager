...
class HealthService {
	...
	/**
	 * Activity feed logger used to date the most recent backup.
	 *
	 * @var ActivityService
	 */
	private $activity;
	...
	/**
	 * Pick the one-line verdict shown beside the health score.
	 *
	...
	/**
	 * Build the dashboard summary cards from the raw metrics.
	 *
	 * @param array $m Metrics.
	 * @return array
	 */
	private function cards( array $m ) {
		...
		// No trailing arrow glyph — DashboardScreen.js renders a real <Icon>
		// after the label instead, so this stays a plain, RTL-safe string.
	...
	/**
	 * Format a byte count, since size_format() returns false for zero — always
	 * give the UI a string.
	 *
	...
	/**
	 * Build the ordered list of cleanup recommendations from the metrics.
	 *
	 * @param array $m Metrics.
	 * @return array
	 */
	private function recommendations( array $m ) {
		...
		if ( $m['expired_transients'] > 0 ) {
			$recs[] = [
				...
				// Navigational label, not "Delete expired" — this link only jumps to
	...
	/**
	 * Render a day count as a human-readable relative phrase.
	 *
	 * @param int $days Days.
	 * @return string
	 */
