<?php
/**
 * DataTables-backed manager for the wp_usermeta table.
 *
...
/**
 * Class UsermetaTableManager
 *
 * DataTables view over wp_usermeta with server-side search, sort and
 * pagination, plus edit and delete for individual meta rows. Keys listed as
 * protected are surfaced read-only so core user data cannot be altered here.
 */
class UsermetaTableManager extends BaseTableManager {


	/**
	 * Bind the manager to the wp_usermeta table.
	 */
	...
	public function get_data() {
		...
		foreach ( $data as &$row ) {
			$is_protected   = in_array( $row['meta_key'], $this->get_protected_usermetas(), true );
	 ...
	 * @throws \Exception When the nonce, permissions, user id or meta key are invalid, or the key is protected.
	 */
