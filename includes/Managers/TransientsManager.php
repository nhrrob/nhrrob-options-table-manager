<?php
/**
 * Lists and manages regular and network-wide transients.
 *
...
class TransientsManager extends BaseTableManager {
	 ...
	 * @throws \Exception If the nonce/permissions check fails or no name was given.
	 */
	public function delete_record() {
		$this->validate_nonce( isset( $_POST['nonce'] ) ? $_POST['nonce'] : '' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.NonceVerification.Missing -- nonce checked in validate_nonce() above
		$this->validate_permissions();

		$name = isset( $_POST['name'] ) ? sanitize_text_field( wp_unslash( $_POST['name'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce checked in validate_nonce() above
		if ( empty( $name ) ) {
	 ...
	 * @throws \Exception If the nonce/permissions check fails or the scope is invalid.
	 */
	public function bulk_delete() {
		$this->validate_nonce( isset( $_POST['nonce'] ) ? $_POST['nonce'] : '' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.NonceVerification.Missing -- nonce checked in validate_nonce() above
		$this->validate_permissions();

		$scope = isset( $_POST['scope'] ) ? sanitize_text_field( wp_unslash( $_POST['scope'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce checked in validate_nonce() above
